...
async function loadCurrentUserForGame() {
  try {
    ...
    currentUser = {
      ...
      hostTotalTaps: Number(data.hostTotalTaps || 0),   // ← new
  isHost: data.isHost === true,                     // ← new (important)
...
async function endSessionRecord() {
  ...
  try {
    await runTransaction(db, async (transaction) => {
      ...
      if (!snap.exists()) {
        console.warn("User document missing — skipping save");
        return;
      }
      ...
      hostTapsDaily[dailyKey]   = (hostTapsDaily[dailyKey]   || 0) + sessionTaps;
      hostTapsWeekly[weeklyKey] = (hostTapsWeekly[weeklyKey] || 0) + sessionTaps;
      hostTapsMonthly[monthlyKey] = (hostTapsMonthly[monthlyKey] || 0) + sessionTaps;
      ...
      transaction.update(userRef, {
        ...
        hostTapsMonthly: hostTapsMonthly,
      });
    });
    ...
    console.log(`%cHost taps saved: +${sessionTaps}`, "color:#ff00aa");
...
// ======================================================
// FETCH LEADERBOARD — HOSTS ONLY VERSION
// ======================================================
async function fetchLeaderboard(period = "daily") {
  ...
  try {
    ...
    // Use HOST fields instead of normal taps fields
    const fieldPath = period === "daily" ? `hostTapsDaily.${key}` :
                     period === "weekly" ? `hostTapsWeekly.${key}` :
                     `hostTapsMonthly.${key}`;
    ...
    snap.forEach(doc => {
      const d = doc.data();
      // Pull taps from host-specific fields
      const taps = d.hostTapsDaily?.[key] ?? 
                   d.hostTapsWeekly?.[key] ?? 
                   d.hostTapsMonthly?.[key] ?? 0;

      if (taps > 0) {
        topScores.push({
          uid: doc.id,
          chatId: d.chatId || "Host",
          taps,
          gender: d.gender,
    ...
    // — MY RANK: Smart & Efficient —
    let myTaps = 0, myRank = null;

    if (currentUser?.uid) {
      // Check if I'm already in the top 13
      ...
      if (inTop !== -1) {
        ...
        // One cheap read for my own data
        const myDoc = await getDoc(doc(db, "users", currentUser.uid));
        if (myDoc.exists()) {
          const d = myDoc.data();
          myTaps = d.hostTapsDaily?.[key] ?? 
                   d.hostTapsWeekly?.[key] ?? 
                   d.hostTapsMonthly?.[key] ?? 0;
    ...
    console.error("Host Leaderboard fetch failed:", err);
...
function renderLeaderboardFromData({ topScores, myTaps, myRank }) {
  ...
  if (display.length === 0) {
    leaderboardList.innerHTML = `<li style="text-align:center;padding:30px;color:#888;">No taps yet — be the first Host!</li>`;
    return;
  }
  ...
  if (currentUser && myRank && myRank > 10) {
...
// ======================================================
// TAB SWITCHER (Unchanged — still works perfectly)
// ======================================================
periodTabs.forEach(tab => {
  tab.addEventListener("click", () => {
    ...
    if (dailyTimerContainer) {
      dailyTimerContainer.style.display = period === "daily" ? "block" : "none";
    }

    // Load the leaderboard (now using host fields)
...
async function saveTap(count = 1) {
  ...
  if (isInBid && window.CURRENT_ROUND_ID) {
    await addDoc(collection(db, "hosttaps"), {
      uid: currentUser.uid,
