...
async function updateLiveBanner() {
  ...
  try {
    ...
    const messages = [
      ...
      { text: "1ST TAKES ₦250K • TAP! IN",                           color: "#FF2D55", glow: true },
...
confirmPlay?.addEventListener('click', async () => {
  ...
  hideActionBar();                    // ← Explicit call
  startSession();                     // your existing function
});

// Also hide when returning to start (if needed)
