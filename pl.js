...
document.getElementById('freeTonightBtn')?.addEventListener('click', async () => {
  ...
  try {
    ...
    // Quick local check first (fast UX)
    ...
    // Check server-side if already active (more reliable than localStorage)
    ...
    // 3. Add location tag (your logic, cleaned)
    ...
    // 4. Add fruit emoji (from user profile)
    ...
    // 6. Deduct stars + save highlights (atomic-ish)
    const batch = writeBatch(db);
...
// ───────────────────────────────────────────────
// Live Countdown Timer (unchanged, but safer)
// ───────────────────────────────────────────────
