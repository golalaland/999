...
/* ================================================
   FREE TONIGHT MODAL – FULL REWRITE (Clean + Strong Filters)
   ================================================ */
function showHighlightsModal(initialVideos, loadMoreFn) {
  ...
  header.style.cssText = `text-align:center; color:#e0b0ff; max-width:640px; margin:0 auto 24px;`;
  header.innerHTML = `
    <div style="background:linear-gradient(135deg,rgba(255,0,242,0.15),rgba(138,43,226,0.12));
                padding:16px 28px; border:1px solid rgba(138,43,226,0.5);
                box-shadow:0 0 20px rgba(255,0,242,0.25); border-radius:16px;">
      ...
      <p style="margin:0; font-size:15px; color:#d0b0ff;">
        Real moments. Real vibes. No paywalls.
      </p>
    </div>
  `;
  ...
  locationBtn.textContent = "Filter by Location";
  ...
  grid.style.cssText = `
    ...
    gap: 16px; width: 100%; max-width: 960px; margin: 0 auto; padding-bottom: 100px;
  `;
  modal.appendChild(grid);

  // State
  let allVideos = [...initialVideos];
  let activeLocation = null;     // Single location filter (strong)
  let activeTags = new Set();    // Other tags
  ...
  function renderCards() {
    ...
    if (activeLocation) {
      visible = visible.filter(v => {
        const loc = (v.location || v.city || "").toLowerCase().trim();
        return loc === activeLocation.toLowerCase();
      });
    }
    ...
    if (activeTags.size > 0) {
      visible = visible.filter(v => {
        const tags = (v.tags || []).map(t => (t || "").trim().toLowerCase());
    ...
    if (visible.length === 0) {
      ...
      empty.style.cssText = "grid-column:1/-1; text-align:center; padding:80px 20px; color:#aaa;";
      empty.innerHTML = activeLocation 
        ? `No clips available in <strong>${activeLocation}</strong> right now.`
        : "No clips found...";
    ...
    visible.forEach(video => {
      ...
      card.style.cssText = `
        ...
        background:#0f0a1a; cursor:pointer; box-shadow:0 6px 25px rgba(138,43,226,0.3);
        transition: all 0.3s ease; border:1px solid rgba(138,43,226,0.3);
      `;
      ...
      card.innerHTML = `
        <div style="width:100%; height:100%; position:relative; background:#000;">
          <video 
            src="${video.previewClip || video.videoUrl || ''}" 
        ...
        <!-- Info Overlay -->
        <div style="position:absolute; bottom:0; left:0; right:0; background:linear-gradient(to top, rgba(15,10,26,0.95), transparent); padding:50px 12px 12px;">
          <div style="color:#00ffea; font-weight:700; font-size:14px;">@${video.uploaderName || 'cutie'}</div>
          <div style="color:#ccc; font-size:12px; margin-top:4px;">
            ${video.location ? video.location : ''}
          </div>
        </div>
        <!-- Free Tonight Badge -->
        <div style="position:absolute; top:12px; right:12px; background:linear-gradient(135deg,#ff3366,#ff9f1c); color:#fff; padding:6px 14px; border-radius:12px; font-size:12px; font-weight:800; box-shadow:0 0 15px rgba(255,51,102,0.8);">
          FREE TONIGHT ♡
        </div>
      `;
      ...
      card.onclick = () => openFullScreenVideo(video.videoUrl || "");
  ...
  function openLocationModal() {
    // ... (I'll keep your nice modal design but make it cleaner)
    const locModal = document.createElement("div");
    locModal.style.cssText = `position:fixed; inset:0; background:rgba(0,0,0,0.85); backdrop-filter:blur(12px); z-index:1000000; display:flex; align-items:center; justify-content:center;`;

    locModal.innerHTML = `
      <div style="background:rgba(20,10,40,0.95); border:1px solid #8a2be2; border-radius:20px; padding:30px; max-width:420px; width:90%; text-align:center;">
        <h3 style="color:#fff; margin-bottom:20px;">Choose Location</h3>
        ...
        <button id="clearLoc" style="margin-top:20px; padding:10px 30px; background:#333; color:#fff; border:none; border-radius:30px;">Clear Filter</button>
      </div>
    `;
    ...
    [...locations].sort().forEach(loc => {
      const btn = document.createElement("button");
      btn.textContent = loc;
      btn.style.cssText = `padding:10px 18px; border-radius:25px; background:rgba(255,255,255,0.1); color:#fff; border:1px solid rgba(255,255,255,0.3); cursor:pointer;`;
    ...
    locModal.querySelector("#clearLoc").onclick = () => {
      activeLocation = null;
      renderCards();
      locModal.remove();
    };
